...
const SpecialOffer = ({ dataAPI, ProductData }) => {
  ...
  useEffect(() => {
    ...
    let temp = dataAPI?.deal_of_days?.deals?.map((deal) => {
      ...
      return (
        ((deal.product = ProductData?.find((product) => product.id === deal.product_id)),
              ...
              ?.map((elem, i) => (
                <div key={i}>
                  <div className="product-image">
                    ...
                    {elem?.product && (
                      <ProductBoxAction productObj={elem?.product} listClass="option" key={i} />
                    )}
                  </div>
                  ...
                  <div className="product-detail text-center">
                    ...
                    <h3 className="price theme-color d-flex justify-content-center">
                      ...
                      <del className="delete-price">
                        {elem?.product?.price ? convertCurrency(elem?.product?.price) : null}
                      </del>
                    </h3>
                    <Progress
