...
const SellerStoreContent = () => {
  ...
  const { data, isLoading } = useQuery(
    [page],
    () => request({ url: StoreAPI, params: { status: 1, page: page, paginate: 9 } }),
  ...
  return (
    <>
      ...
      <WrapperComponent
        classes={{ sectionClass: 'seller-grid-section section-b-space', row: 'g-4' }}
        customCol={true}
      >
        {isSellerLayout == 'basic_store' ? (
          <StoreCard data={data} isLoading={isLoading} setPage={setPage} />
        ) : (
          <ClassicStoreCard data={data} isLoading={isLoading} setPage={setPage} />
        )}
      </WrapperComponent>
    </>
  );
